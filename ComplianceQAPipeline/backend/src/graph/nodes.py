...
def index_video_node(state:VideoAuditState)-> Dict[str,Any]:
    ...
    try:
        ...
        if "youtube.com" in video_url or "youtube.be" in video_url:
            local_path = vi_service.download_youtube_video(video_url,output_oath = local_filename)
        else:
...
def audit_content_node(state:VideoAuditState)->Dict[str,Any]:
    ...
    embeddings = AzureOpenAIEmbeddings(
        azure_deployment = os.getenv("text-embedding-3-small"),
        openai_api_version = os.getenv("AZURE_OPENAI_API_VERSION")
    )

    vector_store = AzureSearch(azure_search_endpoint=os.get_env("AZURE_SEARCH_ENDPOINT")
                                azure_search_key= os.get_env("AZURE_SEARCH_API_KEY")
    ...
    query_text = query_text = f"{transcript} {' '.join(ocr_text)}"
